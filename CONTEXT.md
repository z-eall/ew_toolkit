...
Jere's separate mod providing in-game admin commands and a related data system (e.g. random loot tables) that scripters reference alongside EWP. Treated as a secondary/reference source, not the primary target. Source-verified (2026-08-19, a read of both mods' code) to have **zero code coupling with EWP** — no assembly reference, no `BepInDependency` in either direction. WEC depends on a third mod, `ServerDevcommands`, for its own YAML utilities, not on EWP.
