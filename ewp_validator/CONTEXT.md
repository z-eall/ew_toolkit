...
**Diagnosis category**:
One of the filterable buckets a diagnosis is grouped into: Structure problem, Value problem, Reference problem, YAML problem, Invalid file, Practice recommendation. The naming is deliberate, not arbitrary: `"___ problem"` marks a category that mixes a hard error with a merely-informational finding; `"Invalid ___"` is reserved for the one category that's always a hard error (Invalid file); `"Practice recommendation"` is the one category that's never an error: EWP accepts the input, but a habit is recommended, or part of what was written is silently unused. Info when everything still works, warning when something is unused. Each message opens with a **group** (Legacy format, Legacy filename, Preferred format, Overlapping fields, Silently ignored). "Silently ignored" means EWP loads the script without a fault, but part of what was written never has an effect (for example `==` in a condition) naming the kind of nudge.
