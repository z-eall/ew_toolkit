...
4. **Any object-ownership/`pid`/`cid` change claim** ("who owns this," "claims itself to," "carries the zone host's `<pid>`") — check [EWP_pid_ownership_code_findings.md](docs/guide-source/3-advanced-guide/EWP_pid_ownership_code_findings.md) first, the code-proof ledger built for exactly this. If the component isn't listed there yet, sweep it fresh using the ledger's own Tier A/B/C proof test (the method is in [docs/pid-ownership-proof-method.md](docs/pid-ownership-proof-method.md)) before writing the claim — a field merely *looking* related to an ownership change is not proof it is one. Add the row, sweep-completeness entry included, before the page ships.
