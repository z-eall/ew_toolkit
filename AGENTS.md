...
4. **Every Tool renders the hub's own site-nav bar and matches its real deploy path.** Arriving at any Tool from the hub should feel like switching tabs, not leaving the site: each Tool hand-renders the identical `site-nav` bar (Home / every Tool / Support, current one marked active) using `shared/icons.ts` and `shared/theme.css` — copy `ewp_validator/src/main.ts`'s or `ew_wiki/src/components/Header.astro`'s block rather than reinventing it. Its own build config's `base` must be the full nested path it's actually served at (`/ew_toolkit/<tool>/`), not just the Tool's bare name — a bare-name base breaks every asset and internal link once nested under the hub. If the Tool's own framework keeps a separate theme-persistence key (like Starlight's `starlight-theme`), bridge it to the hub's `ew-toolkit-theme` key on load and on toggle, or the theme silently desyncs between the hub and that Tool.
