...
Shared on purpose: the 5 palette colors, icons, the nav bar, the phone width, and the tokens above. Local on purpose: the validator's error, warning and info colors, and the wiki explainer widgets (`ew_wiki/src/components/*.astro`), which keep their own colors, radii, sizes and fonts and are not scanned by the raw-value test. Still open (design-consistency map): status colors, the symbol rule inside each Tool's own code, and body-style checks in `check:look`.
