...
- Export all no longer freezes the page on large batches; native confirm dialogs are replaced by an in-app confirm. Flagged filenames sit in a dotted, always-on bullet box with a scrollbar when the list is long, so the buttons stay on screen.
