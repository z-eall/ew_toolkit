...
- [Decide repo structure/scaffold and initial bootstrapping](issues/12-repo-scaffold.md) — Layout: `.github/workflows/`, `schema/generate.mjs`, `src/` with a gitignored generated schema, one root `package.json`. MIT license and a minimal README added. Pages source setting needs to be flipped to "GitHub Actions" by the repo owner (one-time, requires their GitHub session).
