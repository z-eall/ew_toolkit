...
- **`CHANGELOG-unreleased.md`, going forward (2026-09-16, see [ticket 08](issues/08-unreleased-log-and-push-reminder.md)):** a repo-root file, auto-appended one raw line per commit — read it before drafting real release notes, it's the "what actually happened" source of truth between cuts. Resets to empty right after a release is cut.

## Decisions so far

- [Unreleased log and push reminder](issues/08-unreleased-log-and-push-reminder.md) — `CHANGELOG-unreleased.md` at the repo root auto-collects one raw line per commit (`guard-changelog-unreleased-log.cjs`, dedupe-safe, seeded from `v2026-08-19` onward) so nothing's lost between release cuts. A separate hook (`guard-changelog-cut-release-reminder.cjs`) asks — advisory, not blocking — before any interactive-session push to `main` when unreleased commits exist. Scheduled/nightly deploys stay untouched, keeping ticket 02's no-CI-involvement rule intact.
