...
**Superseded (2026-09-16) by [The going-forward release-notes format](07-whats-new-changed-bugfixes-format.md):** the diagnosis-category headers (Site, Structure problem, Value problem, ...) are retired in favor of a three-bucket What's New/What's Changed/Bug Fixes structure, applied the same way to every Tool. The pruning-and-plain-language spirit of this ticket's drafting convention still applies; only the section headers themselves changed. Now enforced by a real hook (`guard-changelog-release-format.cjs`), not just written guidance — see ticket 07.
