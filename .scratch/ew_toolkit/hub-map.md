...
- **Confirm-modal component stays deliberately unshared.** `ewp_validator/src/confirmModal.ts`'s own comment flags this: no other Tool has a confirm-style dialog yet, so there's nothing to prove a shared shape against — same discipline the theme-toggle mechanism used to sit under (see [ticket 21](issues/21-unify-hub-tool-nav-bar.md), which just resolved that one). Revisit once a 2nd Tool genuinely needs a confirm dialog.

**Round 2 reopened (2026-09-11)** — the site-nav bar's own hand-copied-3-times pattern (missing `ew_wiki` button on `ewp_validator`, misaligned shape on `ew_wiki`) surfaced during the `ew_wiki` live-bug session; investigating it also surfaced the `.site-nav` CSS block hand-copied 3 times and the theme-toggle mechanism hand-copied 3-4 ways (this map's own fog item above, now graduated). [Ticket 21: unify the hub's site-nav bar](issues/21-unify-hub-tool-nav-bar.md) — resolved. One shared `shared/navBar.ts`/`shared/theme.ts`/`shared/theme.css` implementation now backs all 3 Tools; root causes fixed (a never-updated hardcoded `<a>` list, 2 real conflicting CSS rules, not just a look-alike quirk); `docs/agents/hub-tool-nav.md` written for real and `AGENTS.md` mechanism 4 rewritten to stop telling the next Tool to "copy the block"; `guard-nav-registry-addition.cjs` retired (its premise no longer exists). Verified on live built pages via WSL + browser, not just source diffs. [Ticket 22: unify the hub's favicon](issues/22-unify-favicon.md) — resolved. `ew_wiki` was silently serving Starlight's own generic default icon (no `favicon:` config set), not the toolkit's real hammer icon. `shared/favicon.png` is now the one master, synced into each Tool's `public/` by `scripts/sync-favicon.mjs` (wired as each Tool's `predev`/`prebuild` script, self-healing); `ew_wiki/astro.config.mjs` now sets `favicon: '/favicon.png'` explicitly. Verified byte-identical across all 3 + the built `dist/`.

**Round 3 opened (2026-09-14)** — [Ticket 23: does the Tool-adding mechanism hold up a second time?](issues/23-tool-adding-mechanism-second-pass-review.md) — open, graduated from this map's own fog note now that `ew_wiki` (Tool #2) has shipped; surfaced by a `/wayfinder` review pass.
