...
**Fix (in the Cursor worktree only):** relative script path `./src/prototype/confirm-modal-large-list.ts`; drop HTML `<link>` tags (CSS loads via TS imports); open via `npm run dev:prototype-confirm-modal` which uses the base-prefixed URL.
