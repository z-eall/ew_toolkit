...
- [Standardize font-family across all interactive controls](issues/01-standardize-control-font-family.md) — added a global `button, input, select, textarea { font-family: inherit }` reset right after the `box-sizing` reset in `style.css`; live-verified `.validate-btn`/`.icon-btn` now compute the hub sans-serif stack instead of `Arial`, confirmed no existing explicit font-family rule collides (all higher-specificity class selectors), tests and typecheck pass clean.
- [Fix confirm-modal dashed file-list box sitting flush against its button row](issues/02-confirm-modal-list-box-spacing.md) — added `margin-bottom: 12px` to `.confirm-list-scroll`, matching the modal's existing 12px spacing rhythm; live-verified via injected DOM shape that the gap is now 12px (was 0), tests and typecheck pass clean.
- [Fix misleading diagnosis for unrecognized non-numeric RPC entry keys](issues/03-rpc-unrecognized-key-diagnosis.md) — added `checkRpcUnrecognizedKeys()` (sibling of `checkRpcParams()`) in `rpcValidation.ts`, deriving known-key/rule-entry/spawnData field sets from `schema.generated.json`; flags `triggerRules:`/`remove:`-style keys under `Structure problem` regardless of quoting, suppressing ajv's misleading "must be text" message. Kept the check in `rpcValidation.ts` rather than `shapeMismatchDiagnosis.ts` — matches the existing precedent that per-key RPC checks (unlike whole-entry shape checks) already live and self-suppress there.
